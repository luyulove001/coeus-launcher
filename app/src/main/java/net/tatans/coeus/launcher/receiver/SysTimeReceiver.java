...
public class SysTimeReceiver extends BroadcastReceiver {
	...
	// private TatanCacheUtil weartherSettingCacheUtil;
	private Context ctx;
	...
	// private static boolean isOneFlag = true,isTwoFlag = true;
	...
	public void onReceive(Context context, Intent intent) {
		...
		// "launcher")+ "/";
		// File cachePath = fileUtils.createSDDirs(sdPath, "weather_cache");
		// mCache = TatansCache.get(cachePath);

		System.out.println("进入SysTimeReceiver");
		String action = intent.getAction();
		ctx = context;
		// weartherSettingCacheUtil=new
		// TatanCacheUtil(context,"Weather_Setting");
		initWeatherData();
...
//			TatansToast.showShort(LauncherApp.getInstance(), "天气播报");
			// 发广播给Activity
